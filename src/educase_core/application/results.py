"""Сервис записи/чтения результата прохождения через кодек .eduresult (слой приложения).
...
(ADR-008/ADR-009) — оценка живёт в будущем слое ``report``.
"""
...
from educase_core.domain.attempt import Attempt
from educase_core.infrastructure.archive.codec import read_eduresult, write_eduresult
...
def record_attempt(
    ...
    *,
    ...
    """Упаковать прохождение в .eduresult.
...
def load_result(src: Path) -> LoadedResult:
    """Прочитать .eduresult и собрать доменный ``Attempt`` с ассетами.

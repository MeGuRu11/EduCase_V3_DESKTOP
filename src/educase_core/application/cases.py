"""Сервис загрузки/сохранения кейса через кодек .educase (слой приложения).
...
Результат курсанта (.eduresult) — отдельная будущая фича, здесь не трогается.
"""
...
def load_case(src: Path) -> LoadedCase:
    """Прочитать .educase и собрать доменный ``Case`` с ассетами.
